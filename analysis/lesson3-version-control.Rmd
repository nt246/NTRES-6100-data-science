...
* Feel free to reach out with questions in the `assignments` channel on Slack 
* Nina will host office hours Wednesday 2.30-3pm on Zoom (link will be shared on Slack) 
* Nina will host office hours 11.25-40am (right after class) this Thursday downstairs at the tables outside the Dairy Bar
...
**Acknowledgements**: Today's lecture is adapted from the excellent [R for Excel users](https://rstudio-conf-2020.github.io/r-for-excel/) course by Julia Stewart Lowndes and Allison Horst and the [STAT545 course at UBC](https://stat545.com/).
...
In this course, you will not be working in repositories in your own personal GitHub account, because the instructors need to have access to your files. Instead, you will work within a **personal course repository** hosted in our course GitHub organization, `therkildsen-class`. You will use this same repository throughout the semester to submit all of your assignments and share your work with us.

We will use **Classroom 50** to create your personal course repository. You should have received an invitation to join the course GitHub organization at your **Cornell email address**. Follow the link in that email to accept the invitation. Then follow the Classroom 50 instructions to accept the NTRES 6100 assignment. This will create a private repository for you within `therkildsen-class`. Only you and the course instructors will have access to this repository.

Once you land on your repository page, notice that it is hosted within our course organization, `therkildsen-class`, rather than your personal GitHub account (you can see this in the repository path at the top left). To make your course repository easy to find from your personal GitHub account, click the **Star** button near the top right of the repository page.


```{r, echo=FALSE, out.width="80%"}
knitr::include_graphics("assets/star_repo_26FA.png")  
```

<br>

**You only need to set up your course repo and accept the assignment on Classroom 50 once.** You will use this same repository for all assignments throughout the semester. You are also welcome to use it to keep your class notes, exercises, and other course-related files.

<br>

Now if you click on the profile information in the very top right corner, and select the "Your stars" option, you'll be taken to a list of all the repos you have starred and your course repo should show up here (it should be named something like `therkildsen-class/ntres-6100-course-repo-YOUR_USER_NAME`). Click on the repo name to return to your course repo.
...
#### Together:
* Make sure you're in your class repo (`therkildsen-class/ntres-6100-course-repo-YOUR_USER_NAME`)
...
* Now create a new subdirectory for course notes (remember to not use spaces in names, so you can call it something like course_notes)
...
We should all have set up Git on our local computers by now and have it connected to RStudio. If you don't, follow the instructions [here](https://nt246.github.io/NTRES-6100-data-science/lesson2-quarto-github.html#GitGitHub:_Brief_intro__configuration), and let us know if you run into problems.
...
First, our working directory is set to `~/github/ntres-6100-course-repo-YOUR_USER_NAME`, and `ntres-6100-course-repo-YOUR_USER_NAME` is also named in the top right hand corner.
...
Let's also open up the `README.md`. This is a Markdown file, which is the same language we just learned with Quarto. It's like a Quarto file without the abilities to run code (but we can see much of the formatting in the visual editor). 
...
In my README, right under the line that says: 

```
Throughout the semester, you will add, commit, and push your work to this repository using Git and GitHub.
```
I will write "My name is ... [add your name]."
...
This GitHub repository is now also an RStudio Project (capital P Project). This just means that RStudio has saved this additional file with extension `.Rproj` (ours is `ntres-6100-course-repo-YOUR_USER_NAME.Rproj`) to store specific settings for this project. It's a bit of technology to help us get into the good habit of having a project-oriented workflow. 
...
Let's get a bit organized. First, let's create a new Quarto file for notes that you want to take as we work through the different modules of this course.
...
File > New File > Quarto Document ... (or click the green plus in the top left corner).

Let's set up this file so it's ready for us to enter notes into. I'm going to update the header with a new title and add my name, for now leave the output as html, and then I'm going to delete the rest of the document so that we have a clean start. 
...
Now, let's save it. I'm going to call my file `course-notes.qmd`. 

Notice that when we save this file, it pops up in our Git tab. Git knows that there is something new in our repo. 

Let's also render this file. And look: Git also sees the rendered .html file. 
...
### Output formats for Quarto

After pushing, the rendered html of `course-notes.qmd` file should show up in our GitHub repo after we push it. But how does it look? GitHub just displays the raw html text file, not the nice-looking rendered version we'll see in a browser. 

The nicely formatted files you see on GitHub (e.g. typical README pages) are markdown files (.md in contrast to .qmd). Fortunately, Quarto can output to this format, along with several others including pdf and word documents. We can change the output format in the YAML header at the top of our Quarto document. For the rendered output to display nicely on GitHub, we want to select `output: gfm` (GitHub Flavored Markdown). 
```
---
title: "Notes for NTRES 6100 lectures"
author: "Nina Overgaard Therkildsen"
...
You can find much more information about Quarto output formats [here](https://r4ds.hadley.nz/quarto-formats.html). For most of our work in this course, we will want to use the `gfm` output type because this displays nicely on the GitHub website.

<br>

### Move files to data folder

Now let's try adding a file to our local RStudio project folder so we can push it to GitHub. One of the data files you will need for your next problem set is located [here](https://github.com/nt246/NTRES-6100-data-science/blob/main/datasets/EconomistData.csv). Save this file (using the "Download raw file" icon in the top right of the file display area) into the 'data' subfolder of your R project (the folder you have created for the project (in the cloning process) on your local computer).  
...
* Add a fun fact about yourself or a photo
* Add another line of text with some styling (bolds, italics, headers, bullets etc.)
...
You may have been working on an RStudio project earlier and now you want to add version control through GitHub. You can easily set that up with [these instructions](https://happygitwithr.com/existing-github-first.html) by from Happy Git with R by Jenny Bryan. In fact, we will do this for our [Assignment 2](https://github.com/nt246/NTRES-6100-data-science/blob/main/assignments/Assignment_2.md)!
