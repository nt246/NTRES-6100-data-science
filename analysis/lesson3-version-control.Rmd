...
As we discussed at the end of last class, we have created a GitHub Classroom organization for the class. You should all have accessed your personal course repo through which you will be submitting your assignments and communicating with us. If not, click [here](https://classroom.github.com/a/h81351Rq) and select your name from the list (or just click continue if you don't see your name there).
...
Now if you click on the profile information in the very top right corner, and select the "Your stars" option, you'll be taken to a list of all the repos you have starred and your course repo should show up here (it should be named something like `therkildsen-class/ntres-6100-YOUR_USER_NAME`). Click on the repo name to return to your course repo.
...
#### Together:
* Make sure you're in your class repo (`therkildsen-class/ntres-6100-YOUR_USER_NAME`)
...
We should all have set up Git on our local computers by now and have it connected to RStudio. If you don't, follow the instructions [here](https://nt246.github.io/NTRES-6100-data-science/lesson2-rmarkdown-github.html#GitGitHub:_Brief_intro__configuration)

<br>

## Clone your repository using RStudio

Let's recap on the setup we rushed through at the end of last class. We have all identified our course repo on GitHub, i.e. in the cloud. At the end of last class, we used RStudio to establish it locally on our computers: that is called "cloning". Unlike downloading, cloning keeps all the version control and user information bundled with the files. 

Because our focus last week was just to confirm that our set-up has Git configured and connected to RStudio and GitHub correctly, let's now repeat the cloning process and this time be more intentional about where on our computer we put our local version of the class repo. Cloning is an important task when working with GitHub, so we'll do it over and over again to establish the habit.

Before repeating the cloning step, let's first get rid of the folder we created when we clones the repo last week. Just use your normal file navigation system to do to the folder and delete it. If you can't remember where you put it, you can try searching for `ntres-6100`, which should be the beginning of the folder name. When working with GitHub version-controlled files, we can at any time delete our local copy of a repo and re-clone from the version stored on GitHub. 
...
> **Aside**: HTTPS is default, and [is method recommended by GitHub](https://happygitwithr.com/credential-caching.html#special-consideration-re-two-factor-authentication). You could alternatively set up with SSH, but we will not get into that here.
...
**IMPORTANT**: Make sure to not place it in folder tracked by a cloud storage service (e.g. DropBox, Google Drive or Box) because problem can arise when trying to sync folders with multiple systems.
...
First, our working directory is set to `~/github/ntres-6100-USERNAME`, and `ntres-6100-USERNAME` is also named in the top right hand corner.
...
This GitHub repository is now also an RStudio Project (capital P Project). This just means that RStudio has saved this additional file with extension `.Rproj` (ours is `ntres-6100-USERNAME.Rproj`) to store specific settings for this project. It's a bit of technology to help us get into the good habit of having a project-oriented workflow. 
