...
## Getting set up and reloading the Coronavirus dataset
...
We'll continue working with the coronavirus dataset from the previous lesson. The dataset contains historical COVID-19 records through early 2023. Although it is no longer being updated, its structure makes it useful for practicing data transformation and grouped summaries. First, load the packages we need and the Coronavirus dataset back in directly from the GitHub URL (by loading it from GitHub instead of a file stored on our computer, we're sure to get the most current version).
...
(you can also navigate to it from the `csv` subdirectory and click `View raw` to get the URL)
...
View(vacc) # remember to only run View() interactively from the Console or an R script rather than putting it in a code chunk in a Quarto document that you plan to render.
...
Imagine we want to compare the vaccination rate across countries. We can divide the `people_at_least_one_dose` column with the `population` column to create a new column named `vaxxrate`. We do this with `mutate()` that is a function that defines and inserts new variables into a tibble. You can refer to existing variables directly by name (i.e. without the `$` operator). To make it easier to see what happens, we'll also select just a subset of the variables to retain from our dataset.
...
> Add a new variable that shows the number of doses administered per person who has received at least one dose for each country. First look the patterns across all countries, then only show data for countries that have distributed more than 200 million doses. How many countries is that?
...
**When you're done, sync your R script to Github (pull, stage, commit, push).**
...
>
> 1. Go back to our original dataset `coronavirus` and identify where and when the highest death count in a single day was observed. Hint: you can either use `base::max` or `dplyr::arrange()`.  
...
The last key `dplyr` verb is `summarize()`. It reduces multiple observations to summary values. Without grouping, it typically produces one row; when used with `group_by()`, it produces one summary row for each group. Visually, we are doing this (image from an earlier version of RStudio's dplyr cheatsheet): 
...
#### Which country had the highest count of confirmed cases in January of 2021? [Hint: to address this question the functions `month()` and `year()` from the package lubridate might be helpful]. What about in March?
<br>
