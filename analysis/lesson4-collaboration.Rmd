...
* Next Tuesday's lecture (lesson 5) will also be on Zoom (to facilitate teamwork without worrying about physical distancing). From next Thursday onwards, we'll meet in person, unless you hear otherwise 
...
* We'll need to work in pairs in breakout rooms today, so if you're logged on on multiple devices, please let us know in the `lecture-chat` channel in Slack and type both your participant "names", so we can make sure to put those in the same room with a partner
