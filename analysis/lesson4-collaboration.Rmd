...
There are no required readings for today's lesson, but if you're keen to learn more, check out the other resources listed under [Lesson 3](https://nt246.github.io/NTRES-6100-data-science/lesson3-version-control.html)
...
Let's have Partner 2 make some edits to the README file. We will edit this file and illustrate how Git tracks files that have been modified (to complement seeing how it tracks files that have been added).

In the README, you can write something like: 

`This repo is for our collaborative project` 

When I save this, notice how it shows up in my Git tab. It has a blue “M”: Git is already tracking this file, and tracking it line-by-line, so it knows that something is different: it’s Modified with an M.
...
Within a file, Git tracks changes line-by-line. So you can also have collaborators working on different lines within the same file and Git will be able to weave those changes into each other -- that's its job! 

It's when you have collaborators working on *the same lines within the same file* that you can have **merge conflicts**. This is when there is a conflict within the same line so that Git can't merge automatically. They need a human to help decide what information to keep (which is good because you don't want Git to decide for you). Merge conflicts can be frustrating, but like R's error messages, they are actually trying to help you. 
...
Git provides some guidance: either commit this work first, or "stash it", which temporarily sets aside your uncommitted changes, allowing you to update your repository and then reapply those changes afterward.

Let's follow their advice and have Partner 1 commit. Great. Now let's try pulling again. 

When you first try this, you will likely see this error:  

```{r reconconcile-branches, echo=FALSE, out.width="80%"}
knitr::include_graphics("assets/github_reconcile_branches.png")  
```

<br>

This warning was added in Git 2.27 (i.e. you might not see something similar just as a warning, not an error message in earlier versions). Git makes it mandatory to define the default behavior for reconciliation of branches when merging in git config. Hence, when there is no default method defined, this warning is shown until we define a default behavior.

You can read more about the different options for setting default behavior [here](https://confluence.atlassian.com/bbkb/git-pull-fails-with-a-warning-about-reconciling-divergent-branches-1271693522.html). We're going to set the default behavior to make a merge commit when branches have diverged. We do this by running the following command in our **Terminal** window (not the R console):

`git config --global pull.rebase false`  

<br>
By adding the flag `--global` we are defining the same behavior for all repositories instead of having to do it each time we open a new repository.

<br>
<br>

Now, we're ready pull again. Try that.

<br>

New error: "Merge conflict in README...fix conflicts and then commit the result". 

So this error is different from the previous: Git knows what has changed line-by-line in my file here, and it knows what has changed line-by-line in the version on GitHub.com. And it knows there is a conflict between them. So it's asking me to now compare these changes, choose a preference, and commit. 
...
We will again follow Git's advice to fix the conflicts. Let's close this window and inspect. 
...
So, to resolve this merge conflict, Partner 1 has to choose which one to keep. And I tell Git my choice by deleting everything in this bundle of text except the line they want. So, Partner 1 will delete the `<<<<<<HEAD`, `=====`, `>>>>long commit identifier` and either Julie or Allison's line that I don't want to keep. 
...
To minimize merge conflicts, pull often so that you are aware of anything that is different, and deal with it early. Similarly, commit and push often so that your contributions do not become too unwieldly for yourself or others later on.
...
**Merge conflicts most commonly occur when two people have changed the same lines—or overlapping parts—of the same file.** To clearly illustrate this, we will now demo a series of different outcomes we can experience when two people are working on the same repo and end up making changes in parallel without pushing in sequence (i.e. partners end up with different versions on their local computers that need to be reconciled in the remote repo).  
...
Let's have Partner 2 create a new Quarto file and name it `analysis.qmd`. Here's what you will do: 

1. Pull! (in case Partner 1 has pushed something in the meantime)
2. Create a new Quarto file and name it `analysis.qmd`. Choose html as the output format if you're using the "Create Quarto document wizard, but change it to `format: gfm` in the file (GitHub Flavored Markdown, to display nicely on GitHub. 
3. Change the title inside the .qmd file, call it "Our Collaborative Analysis"
...
Now you both have a copy of the `analysis.qmd` file. Partner 2 can add a new header just before the `Running Code` header (e.g. line 11), then pull, stage, commit, push.

Now Partner 1 can add a note under the last code chunk, around line 27. Then try to pull. Then commit, and pull, stage, push.
...
This is where problems arise. Now Partner 2 makes a change to the first code chunk. Change `1 + 1` to something else, e.g. `7 + 8`. Then pull, stage, commit, push

Partner 1 should not pull yet, but instead also make a change to that first code chunk, so change `1 + 1`, e.g. to `3 + 4`. Then try to pull (will be rejected). Then stage, commit and pull. 
...
Obviously, you should push work to the shared branch because it’s “ready” to share (or at least “ready enough”), not to avoid Git merges.
...
Your early experiences collaborating with others and yourself in the main branch will give you a visceral understanding of why most Git users eventually start to use branches.
