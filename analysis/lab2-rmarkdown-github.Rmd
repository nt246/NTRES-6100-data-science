...
## Exercise 1: Improve your CV (20 minutes)
...
## Exercise 2: Improve your CV using dedicated packages with the help of AI (25 minutes)

While custom formatting in Quarto can be powerful, the Quarto community has dedicated templates designed specifically to generate professional-looking CV's. 

<br>


Some options include `typstcv` and `schochastics/quarto-cv`. You can install either in the terminal using:
...
For this next section of the lab, install one of the above templates, migrate some of your information from your existing CV to the template, and use an AI agent for help with formatting and troubleshooting. 

<br>

Begin by familiarizing yourself with the following *before* requesting any AI help:
- An example `qmd` file to understand the formatting and syntax expected
- Check online documentation for the respective templates for more information on how to configure the template for your needs

Use what you find on your own to guide your prompts to your chosen LLM. Specific prompts provided with context for your needs and the templates/packages you are using are crucial to ensuring your outputs are accurate and useful.

If you generate something you like, please share your results to `#lab-chat` on Slack!
...
## Exercise 3: Peer review your CV (45 minutes)

<br>

#### Issues and projects

Issues and projects are useful GitHub features that can facilitate your project management, especially in collaborative projects. They can be used for providing feedback, making plans, assigning tasks, and tracking progress. We will explore these useful GitHub features, please follow this detailed  [tutorial](https://fish497.github.io/website/lectures/week_02/lec_04_intro_github.html#Issues). 
...
1. Use the intentional typos as an opportunity to practice how to resolve merge conflicts, and how to avoid them when possible. 
    a. First, both partners could correct a same typo in one partner's `qmd` file in slightly different ways. **Do not render the `qmd` file yet.** Then, both partners can commit their changed `qmd` file. One partner can push first, and the other partner will pull, and will get a merge conflict. They can then address this merge conflict, **render**, push the resolved version to GitHub, and the other partner can pull. After this, partners can switch roles. 
    b. Then, partners can correct typos in different lines in one person's `qmd` file. **Similarly, neither should render at this point.** Again, they will both commit their changes, one partner will push first, and when the other pulls, the changes should be merged automatically. They can now render, push, and the other partner can pull. 
