...
* We're half way through the course! We will send out a link to a mid-term evaluation form (we'll use our own form because the official CALS version won't be ready in time to work with our schedule). We would appreciate your candid feedback so we can make the most of our remaining time together
...
* Load the packages we'll use here (you will have to install `janitor` if you don't already have it):
    - `tidyverse`
    - `janitor`
...
Now that the motivation for wanting to work with data in tidy format hopefully is clear, let's explore some powerful functions from the package `tidyr` for reshaping data. (`tidyr` comes bundles with the `tidyverse`, so we don't have to install it separately). 
...
### More exercises on the LOTR data (you can do these on your own)
...
Earlier versions of the coronavirus dataset we've been working with had different variables names. You can see those by reading in a version from April 2020.

```{r}
coronavirus <- read_csv('https://raw.githubusercontent.com/nt246/NTRES-6100-data-science/main/datasets/coronavirus_apr04-2020.csv', col_types = cols(Province.State = col_character()))

```


You'll notice that the variable names here have a very inconsistent format. It's not a problem per se for our ability to work with this dataframe, but it's a bit annoying to look at.
...
Here, we'll use `janitor::clean_names()` to convert all of our column headers in our old `coronavirus` dataset to a more convenient case. The default is **lower_snake_case**, which means all spaces and symbols are replaced with an underscore (or a word describing the symbol), all characters are lowercase, and a few other nice adjustments. 
