...
* We're half way through the course! We will send out a link to a mid-term evaluation form. We would appreciate your candid feedback so we can make the most of our remaining time together
...
The datasets come with the `tidyverse` package, so you can access them by just typing their names, e.g. `table1`, `table2` etc. We computed the **infection rate per year 10,000 people for each year and each country** from `table1` together using our now familiar `dplyr` wrangling tools. Now calculate this same statistic from `table2` and `table4a` + `table4b`. You will need to perform four operations:
...
And now that you're over in "R for Data Science", you can continue on to also take a look at [Chapter 12.4](https://r4ds.had.co.nz/tidy-data.html#separating-and-uniting) on `separate()` and `unite()` - two simple functions for splitting and combining information from different columns.
