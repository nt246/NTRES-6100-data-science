---
title: "Lesson 5: Collaborating with GitHub - Part 2: Making websites with GitHub Pages"
output: 
  html_document:
    toc: true
---
...
There are no required readings for today, but if you did not join our lecture last Thursday, please read through the [lecture notes](https://nt246.github.io/NTRES-6100-data-science/lesson4-collaboration.html) and watch the recording posted on [Canvas](https://canvas.cornell.edu/courses/88419)
...
* Thanks for the feedback! Feel free to share additional suggestions
* Assignment 2 is due this Thursday - let us know if you have any questions
* You will do assignment 3 (due Thursday Sept 17) in collaboration with the partner you get assigned today, so make sure to exchange contact info before you leave class
* Let's add out name and photo to our Slack accounts to make the workspace more personal (we'll likely start using it more in the coming weeks)
...
There are several different ways we can set up a website with Quarto. To keep practicing the workflow we used during last class, we'll here use a "GitHub first" approach. Later on, you can try on your own to set up a similar project using an RStudio project first approach, e.g. following [this tutorial](https://ucsb-meds.github.io/creating-quarto-websites/#create-website-scaffolding) [and you can read more about the differences between the two approaches in [Chapters 15-17 in Happy Git with R](https://happygitwithr.com/new-github-first)] 
...
Now it's going to get fun. Let's start by making a super simple website. Quarto has built-in functionality to create all the basic files we need for a simple website. Because we have already created and cloned our GitHub repository, we'll create the Quarto website inside our existing project using a command in the **Terminal**. Quarto is fundamentally a command-line tool, meaning that we can give Quarto instructions directly by typing commands in the Terminal. In RStudio, the **Console** and **Terminal** look similar, but they communicate with different programs: the Console runs **R commands**, whereas the Terminal runs **commands for your computer's operating system and other command-line programs, such as Quarto and Git**.

Open the **Terminal** tab in the bottom-left pane of RStudio (**not the Console**) and have Partner 2 type and run the following command:

`quarto create-project --type website`  

[only Partner 2 should do this]

This adds the default files (`_quarto.yml`, `index.qmd`, `about.qmd`, and  `styles.css`) that we need for getting started on our website. 
...
Create an empty file named `.nojekyll` in your repository’s root directory (e.g. `collab-website/`), which is required to disable some processing of HTML files that GitHub does by default. The presence of a `.nojekyll` file tells GitHub Pages to serve the files in the current folder directly rather than processing them with Jekyll.

From RStudio’s Files pane, click New File > Text File, then type .nojekyll > click OK (you can use the Text File option to create any file type).
...
>Note: Your website currently exists only on your local computer. You can preview it in RStudio or your browser, but it isn't published on the web with a searchable URL yet. We’ll get there soon though!

But first, Partner 1 must save, stage, commit, and push their changes to GitHub.
...
You can also always find the website by going back to "Settings", the "Pages" (in the left-hand menu) within your GitHub repo.
...
The old version of the website may also be stored in your browser's cache, so if it has not updated after 1-2 minutes and you confirm that you've pushed your changes to GitHub, trying viewing the URL in a different browser or an incognito window.  
...
In the default Quarto website skeleton, there are two items in the navbar that appear as “Home” and “About.” Those navbar tabs link to two `.html` files (`index.html` and `about.html`) and are automatically rendered when you Build to `docs/`. Adding a new page to your website requires two steps:
...
From RStudio’s Files pane, click Blank File > Quarto Document > give it a name (e.g. resources.qmd and save it to your project’s root directory
...
Render your website and see that this creates a newly-added navbar page. Note: The page name, as it appears in the navbar of your website, will be the same as whatever is listed in the title field of that file’s YAML. For example, I have set title: "All my favorite resources!" in the YAML of resources.qmd – this is how it will appear in my website’s navbar. If you’d like to set the navbar name as something other than the page’s title, use the href and text options together (e.g. see how index.qmd is rendered as Home in my website’s navbar).

Save, commit, stage, and push your changes to GitHub and inspect the results.

<br>

## Change the theme

Update the appearance of your site by choosing from one of the 25 predefined [Bootswatch themes](https://bootswatch.com/). By default, Quarto sites are built using the cosmo theme. Supply just one theme name to the theme option in your `_quarto.yml file`:
...
Or any other settings described in the [Quarto Documentation](https://quarto.org/docs/websites/).
...
**You now have one project that combines Quarto + RStudio + Git + GitHub + GitHub Pages.**
...
https://nbisweden.github.io/workshop-data-visualization-r/2605/
https://lse-dsi.github.io/ME204/2025/index.html?utm_source=chatgpt.com
...
One important thing to note is that we have exclusively interacted with Git through RStudio because that provides a really nice interface and integrates well with Quarto and other work in R. However, there are some limits to the functionality, so for more advanced usage you will probably have to either use the [GitHub Desktop program](https://desktop.github.com/) or interact with Git and GitHub through the command line.
