...
For this lesson, we will work with a dataset of reported COVID-19 cases and deaths from countries around the world from 2020–2023. This dataset provides a useful real-world example for learning data wrangling because it contains observations across countries, dates, and types of records, giving us plenty of opportunities to practice selecting variables, filtering observations, and combining multiple data-wrangling operations.

The basic structure and context of the data are also relatively intuitive, allowing us to focus on learning the tools for manipulating data rather than spending a lot of time understanding an unfamiliar dataset. It will illustrate how data science tools can help us explore patterns in complex datasets and improve our understanding of global challenges.
...
We will use a dataset compiled in the [`coronavirus` R package](https://github.com/RamiKrispin/coronavirus) developed by Rami Krispin. The dataset is based on data collected by the Johns Hopkins University Center for Systems Science and Engineering (JHU CSSE) and includes daily reported COVID-19 cases and deaths from January 2020 through March 2023, as well as recovery data through August 2022.

The coronavirus dataset is available through the R package, but for this exercise we will read the data directly from a CSV file hosted on GitHub. This gives us an opportunity to practice importing a dataset directly from the web rather than first downloading it to our computer.

Navigate to the [csv folder in the coronavirus GitHub repository](https://github.com/RamiKrispin/coronavirus/tree/master/csv) and click on coronavirus.csv. Because the file is large, GitHub may not display the entire dataset in its usual preview. Click Raw to view the raw CSV file.

Copy the URL from the raw data page. It should look like this:

https://github.com/RamiKrispin/coronavirus/blob/master/csv/coronavirus.csv

Now, let's go back to RStudio. In our R script, let's read this csv file and name the resulting dataset "coronavirus". We will use the `read_csv()` function from the `readr` package (part of the tidyverse, so it's already installed!). 
...
In tidyverse syntax, there are also other ways to pull out single variables (e.g. with the `pull()` and `select()` functions. We'll get to those later after we have introduced basic dplyr workflows.
...
How about if we want two country names? We can’t use a single instance of the == operator here, because we want to match either of two different values. Instead, we can combine two comparisons using [Boolean operators](https://r4ds.hadley.nz/logicals.html#boolean-algebra) : & means “and”, | means “or”, and ! means “not”. So if we want records from either the US or Canada, we can type:
...
> **1a:** Subset the data to only show the death counts in three European countries on today's date in 2021.
> 
> **1b:** Create a dataset containing only reported deaths from the US and Canada. How many observations does the resulting dataset contain?
>
**OPTIONAL: 1c:** What is the total number of deaths in the US reported in this dataset (up to March 2023)?   
> Hint: You can do this in 2 steps by assigning a variable and then using the `sum()` function.



> Then, sync to GitHub (pull, stage, commit, push).
...
filter(coronavirus, country %in% c("Denmark", "Italy", "Spain"), type == "death", date == "2026-09-15")
...
Before we go any further, we should explore the pipe operator `|>`. If you have not used this before, **this is going to change your life** (at least your coding life...). You no longer need to enact multi-operation commands by nesting them inside each other. And we won't need to make temporary variables like we did in the US example above. This new syntax leads to code that is much easier to write and to read: it actually tells the story of your analysis.

<br>

> The tidyverse originally used the `%>%` pipe provided by the [`magrittr package`](https://github.com/smbache/magrittr) package, so if you've been using tidyverse for a while, you may have been used to seeing and implementing this yourself. The magrittr package is included in the core tidyverse, so you can use `%>%` whenever you load the tidyverse. However, R version 4.1.0 introduced a native pipe operator `|>`, which is now the recommended syntax (more details [here](https://r4ds.hadley.nz/data-transform.html#sec-the-pipe)). The course notes should be fully updated to use the native R pipe `|>`.
...
* It is not self-documenting. What are the columns we're retaining here?
...
**Save your R script (render if you've been working in a Quarto file), and sync it to GitHub (pull, stage, commit, push)**
...
If we have time, we'll explore some [additional options for `select()`](https://r4ds.hadley.nz/data-transform.html#sec-select)
