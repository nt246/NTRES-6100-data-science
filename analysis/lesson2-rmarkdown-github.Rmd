...
First, let us each access our course repo. We have created a GitHub Classroom organization for the class. To access you personal course repo through which you will be submitting your assignments and communicating with us, click [here](https://classroom.github.com/a/h81351Rq)
...
For now, GitHub still allows authentication with your user name and password, so you *can* proceed without this step. However, from August 2021 [GitHub will require token- or SSH-based authentication](https://github.blog/2020-12-15-token-authentication-requirements-for-git-operations/), so let's get set up with that right away so your workflows won't be interrupted when the standard username-password authentication stops working.

We will set up using HTTPS and a Personal Access Token (PAT) rather than SSH. If you want to know why, check out recommendations in [Happy Git with R](https://happygitwithr.com/credential-caching.html) or in [this usethis vignette](https://usethis.r-lib.org/articles/articles/git-credentials.html), but you can also just take our word for it.
...
Make sure you're signed into your (www.github.com) in a web browser and that you have [verified the email address associated with your account](https://docs.github.com/en/github/getting-started-with-github/verifying-your-email-address). 

Now run `create_github_token()` in your RStudio console. This will take you to a pre-filled form to create a new PAT. Alternatively, you can get to the same page from ["Settings" in your GitHub account](https://docs.github.com/en/github/authenticating-to-github/creating-a-personal-access-token) in the browser. The advantage of running `create_github_token()` is that the developers of the `usethis` package have pre-selected some recommended scopes, which you can look over and adjust before clicking “Generate token”.




You can generate a PAT under the ["Settings" from your GitHub browser page](https://docs.github.com/en/github/authenticating-to-github/creating-a-personal-access-token).

by clicking on “Generate new token” from https://github.com/settings/tokens. 
