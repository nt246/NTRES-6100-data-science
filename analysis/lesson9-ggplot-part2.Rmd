...
coronavirus %>% 
  ...
  summarize(cases=sum(cases)) %>%
  ggplot() +
  geom_col(aes(x=date, y = cases, fill = type), width = 1) +
  facet_wrap(~country)
```
...
coronavirus %>%
  ...
  summarise(count=n()) %>%
  ggplot() +
  geom_line(mapping = aes(x = date, y=count), color="black")
```
...
We can remove countries with a small number of confirmed cases, and log transform the case counts

```{R, eval = TRUE}
coronavirus_ttd %>%
  filter(confirmed > 2000000) %>%
  ggplot() +
  geom_label(mapping = aes(x = log(confirmed), y = log(death), label = country))
```

Or, we can zoom in on the region with low case counts

```{r}
coronavirus_ttd %>%
  filter(confirmed > 0, confirmed < 20000) %>%
  ggplot() +
  geom_label(mapping = aes(x = confirmed, y = death, label = country))
```
