...
The second approach is to prespecify the type and length of the output before the loop, and fill in the output in each iteration, such as the following:
...
*Hint: the `system.time()` function may be helpful*

*Hint: try to compare the runtime of the two approaches with different numbers iterations*
...
while(i<=n){
  if(i==1){
    ...
    hysterical_monkey <- paste0(hysterical_monkey, current_letter)
  }
